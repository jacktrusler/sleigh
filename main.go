...
func main() {
	...
	year := flag.Int("year", CurrentYear, "Year of the problem (2015-2024).")
	day := flag.Int("day", today, "Day of the problem (1-25).")
	name := flag.String("name", fileName, fmt.Sprintf("Optionally set a filename, defaults to %s", fileName))
	flag.Parse()
	...
	if *year > CurrentYear || *year < 2015 {
		fmt.Printf("Year is out of range, please enter a year between %d and %d, if left blank, defaults to current year (%d).", 2015, CurrentYear, CurrentYear)
		return
	}
	if *day > 25 || *day < 1 {
